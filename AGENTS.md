...
- **Several tasks at once.** Up to three tasks run at the same time, each in a different app
  (`Core/Task/ConcurrentTaskPolicy`). Each is a `TaskSession` (`App/TaskSession`) with its own state, backend,
  cursor (in its own color, `Core/Cursor/TaskColorPalette`), checklist panel, window observer, visibility monitor,
  takeover detector and pending questions. Summoning Dotto over an app that already has a task going opens that
  task's checklist instead; a finished task's summary is replaced by its app's next task. Recording a demonstration
  blocks new tasks, since it records every click and key. What only one task may use at a time is shared and
  counted: the foreground assist takes turns (`ForegroundAssistTurnQueue`), Accessibility modes are held per app
...
3b. **Dotto never takes focus.** No activation, no raising, no cursor moves, no private focus tricks that could
   ...
   Chromium or Electron tree live, which is removed notification by notification) are held by each task's own
   backend and counted per app (`Core/InputDelivery/AccessibilityModeHolds`): they go on with the first hold and are
   restored with the last release, at the task's end, on
...
| Mock model (Debug builds only) | UserDefaults `dottoMockClaudeTransport` (`defaults write com.sryo.dotto dottoMockClaudeTransport -bool true`): `Platform/Claude/MockClaudeTransport` answers every request from `Core/Claude/ClaudeMockConversationScript` (read the window, a two-item plan, one harmless action per item) and no request reaches Anthropic, so no key is needed. Compiled out of Release builds |
...
|---|---|
| `Dotto/App/DottoApp.swift` | `DottoApp` and `DottoAppDelegate`, the composition root. Builds the transport, monitors, store and panels, and the factory that gives every task its own `AccessibilityActionBackend` (sharing the element reader, synthesizer, capturer, window-server bridge and one `TargetApplicationAccessibilityModes`) |
| `Dotto/App/TaskSessionController.swift`, `TaskSession.swift`, `TaskSessionScope.swift` | The coordinator: its published app-wide state, its sessions (`sessionForNewTask`, `withSession`, `focus`) and the state machine events it applies. Everything that belongs to one task (state, target app, audit log, Stop signal, budget, backend, planner conversation, run control, pending questions, takeover detector, cursor, checklist panel, observers, color) lives in its `TaskSession`; the controller forwards the current session's fields under their old names. `TaskSessionScope` is one session's view of the coordinator, for its panels |
...
| `Dotto/Core/Task/` | `TaskSessionStateMachine` (every legal state transition), `PendingUserAnswer` (a run's one outstanding question, answered exactly once from any surface), `ConcurrentTaskPolicy` (which session a new task goes to: three tasks at most, one per app), run summary, user-facing messages |
...
| `Dotto/Core/Display/`, `Dotto/Platform/Display/` | `PrimaryDisplayHeightResolution` and `PrimaryDisplayHeightCache` (the primary display height for top-left ↔ AppKit flips, never 0: the last settled height while displays reconfigure) and `DisplayReconfigurationTracker` (Quartz's per-display callbacks as one began and one finished); `DisplayReconfigurationObserver` (`CGDisplayRegisterReconfigurationCallback`, read-only: it records the settled height and posts `didFinishNotification`) and `PrimaryDisplayHeightReader`. Every coordinate flip reads the height through these, never `NSScreen.screens.first?.frame.height ?? 0` |
