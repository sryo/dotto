...
enum PromptLibrary {
    ...
    - Prefer element tools (click, type_text, scroll). Use screenshot and click_point only when the element you need is missing from the outline, for example custom-drawn or inaccessible web content; click_point uses pixel coordinates of the most recent screenshot.
    ...
    - To change text in a text field or text area, don't click it first and don't move the caret: call type_text with its element_id. To edit inside existing text, read the current text, then type the whole edited text with replace_existing_text true; to add at the end, use replace_existing_text false.
