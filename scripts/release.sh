...
# =============================================================================
# release.sh — Automates the full release pipeline for Dotto
#
...
DMG_BACKGROUND="${PROJECT_DIR}/scripts/release-assets/dmg-background.png"

GITHUB_REPO="sryo/dotto"
...
# same-named DMG so create-dmg doesn't choke on duplicates.
...
echo "📦 Archiving..."
xcodebuild archive \
    -project "${PROJECT_DIR}/Dotto.xcodeproj" \
    -scheme "${SCHEME}" \
...
gh release create "${TAG}" "${DMG_PATH}" \
    ...
    --notes "Dotto v${MARKETING_VERSION}" \
    --latest
